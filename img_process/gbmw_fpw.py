...
class GBMW_FPW():
    ...
    def get_images(self):
        ...
        # pred_dir = pathlib.Path.cwd() / "data" / "test" / "labels" ## zaq
        ...
        predicted = [np.invert(imread(tar_name)) for tar_name in predicted_names]  # inverted???
        # predicted = [imread(tar_name) for tar_name in predicted_names]  # inverted???
    ...
    def save_results(self, cur_fold, cur_model):
        ...
        self.reset_dir([save_dir, blur_dir, orig_dir, pred_dir, label_dir, compare_dir])
    ...
    def calc(self, cur_fold, cur_model):
        ...
        for i in range(0,len(predicted)):
            predict = predicted[i]
            flip_predict = np.invert(predict)
            # flip_predict = predict
            orig = original[i]
            #  gaussian filtering and Otsu-thresholded
            ...
            for j in range(1,num_segments+1):
                ...
                if num_edges == 2:
                    ...
                    try:
                        ...
                        indicate[k] = 0
                    if indicate[0]==0 and indicate[1]==0:
                        tile_segs_num_sd.append(0)
                        tile_segs_mean_fpw.append(0)
                    elif indicate[0] > indicate[1]:
    ...
    def save_csv(self, images_names, gbmw_nm, fpw_nm, jaccard, jaccard_blur):
        ...
        for key in target_image_data.keys(): # RuntimeWarning: Mean of empty slice.
